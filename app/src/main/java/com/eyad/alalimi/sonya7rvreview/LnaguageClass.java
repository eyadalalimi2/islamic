...
public class LnaguageClass {
    ...
    // public static int ISBackgroundMusic = 1; // 1 for is not, 20 for out back
    // ground 3 for in background
    ...
    public ArrayList<AuthorClass> AutherList() {
        ...
        if (SaveSettings.LanguageSelect == 1) {
            ...
            AutherListInfo.add(new AuthorClass("maher", " ماهر المعيقلي"));
            ...
            AutherListInfo.add(new AuthorClass("sayed", " sayed muslim"));
    ...
    public ArrayList<AuthorClass> GuranAya(String ReciteName) {
        ...
        for (int i = ListRange.beginR; i < ListRange.endread; i++) {
            try {
                ...
                // String[] fmyFilemyFileiles = isoStore.GetFileNames(RealServerFolder +
                // ListAya[i].ServerName + ".mp3");
                File myFile = new File(AYAPAth);
                if (myFile.exists())
                    ListAyaRanage.add(new AuthorClass(ac.ServerName, ac.RealName, avalible(), AYAPAth));
                else {
                    AYAPAth = "http://server" + serverNumber(ReciteName) + ".mp3quran.net/" + ReciteName + "/"
                            + ac.ServerName + ".mp3";

                    // AYAPAth = "http://www.quran.alrubaye.com/quran/" + LnaguageClass.RecitesName
                    // + "/" + ac.ServerName + ".mp3";
