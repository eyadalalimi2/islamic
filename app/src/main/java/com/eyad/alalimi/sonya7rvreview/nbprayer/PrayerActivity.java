    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (!pm.isIgnoringBatteryOptimizations(packageName)) {
                ...
                builder1.setMessage(
                        "ليشتغل معك الاذان أو الاشعارات التلقائية بدون اي مشاكل تاكد انك جعلت التطبيق يشتغل في الخلفية لذا يرجى تفعيل هذه الخاصية لكي لاتواجهك اي مشاكل");
        ...
        if (ActivityCompat.checkSelfPermission(activity,
            ...
            if (withRequest) {
                ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.ACCESS_COARSE_LOCATION,
                        Manifest.permission.ACCESS_FINE_LOCATION}, ACCESS_COURSE_LOCATION_CODE);
            } else
