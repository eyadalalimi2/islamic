...
public class HijriDate {
    ...
    public static String writeIslamicDate(Context context, Calendar today, long oldDate) {
        ...
        String outputIslamicDate = wdNames[calendar.get(Calendar.DAY_OF_WEEK) - 1] + ", " + (iDate[5]) + " "
                + iMonthNames[(int) iDate[6]] + " " + iDate[7];
    ...
    public static String convertToHijriDate(Context context, Calendar cl) {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                ...
                HijrahDate islamyDate = HijrahChronology.INSTANCE.date(java.time.LocalDate.of(cl.get(Calendar.YEAR), cl.get(Calendar.MONTH) + 1, cl.get(Calendar.DATE)));
                String dateStr = day + " , " + DateTimeFormatter.ofPattern("dd MMMM yyyy", new Locale("ar")).format(islamyDate);
...
//        LocalDate todayIso = new LocalDate(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1, calendar.get(Calendar.DAY_OF_MONTH), iso);
...
//        LocalDate todayHijri = LocalDate.fromCalendarFields(calendar.getTimeInMillis(), hijri);
        Log.d(TAG, "onCreate:todayHijri " + todayIso);
        String outputIslamicDate =
                wdNames[todayHijri.getDayOfWeek() - 1] + ", " +         // اسم اليوم
